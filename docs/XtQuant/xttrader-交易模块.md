...
> 提示：市价类型只在实盘环境中生效，模拟环境不支持市价方式报单
...
| 属性 | 类型 | 注释 |
|---|---|---|
| account_type | int | 账号类型，参见数据字典账号类型(account_type) |
| account_id | str | 资金账号 |
| cash | float | 可用金额 |
...
| order_time | int | 报单时间 |
| order_type | int | 委托类型，参见数据字典委托类型(order_type) |
| order_volume | int | 委托数量 |
| price_type | int | 报价类型，该字段在返回时为柜台返回类型，不等价于下单传入的price_type，枚举值不一样功能一样 |
| price | float | 委托价格 |
| traded_volume | int | 成交数量 |
| traded_price | float | 成交均价 |
| order_status | int | 委托状态，参见数据字典委托状态(order_status) |
| status_msg | str | 委托状态描述，如废单原因 |
| strategy_name | str | 策略名称 |
| order_remark | str | 委托备注，最大 24 个英文字符 |
| direction | int | 多空方向，股票不适用；参见数据字典多空方向(direction) |
| offset_flag | int | 交易操作，用此字段区分股票买卖，期货开、平仓，期权买卖等；参见数据字典交易操作(offset_flag) |
...
| account_type | int | 账号类型，参见数据字典账号类型(account_type) |
| account_id | str | 资金账号 |
| slo_status | int | 融券状态（参见下一条融券状态说明） |
| fin_status | int | 融资状态 |
| exchange_id | int | 证券市场 |
...
| account_type | int | 账号类型，参见数据字典账号类型(account_type) |
| account_id | str | 资金账号 |
| status | int | 账号状态，参见数据字典账号状态(account_status) |
...
| login_status | int | 账号状态，参见数据字典账号状态(account_status) |
