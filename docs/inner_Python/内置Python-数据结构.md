...
| `openInt` | `int` | 若是股票，则openInt含义为股票状态，非股票则是持仓量 |
...
| m_dInstrumentValue | float | 总市值，合约价值 |
| m_dDeposit | float | 入金 |
...
| m_dBuyWaitMoney | float | 买入待交收金额（元） |
| m_dSellWaitMoney | float | 卖出待交收金额（元） |
...
> 由于字段m_dSloRatio、m_dSloStatus提供来源和取担保品明细 **get_assure_contract** 重复，字段在2021年9月移除，后续用担保品明细接口获取。
...
|--------|----------|------|
| m_nTaskId | int | 任务号 |
| m_eStatus | enum | 任务状态 ETaskStatus类型 |
...
| m_eOperationType | enum | 下单操作：开平、多空……EOperationType类型 |
| m_eOrderType | enum | 算法交易、普通交易 EOrderType类型 |
| m_ePriceType | enum | 报价方式：对手、最新…… EPriceType类型 |
