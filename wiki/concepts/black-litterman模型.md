---
title: "black-litterman模型"
title_zh: "Black-Litterman模型"
type: concept
summary: "Black-Litterman模型（BL模型），在贝叶斯框架下融合先验收益率与投资者观点形成后验分布，解决均值方差模型对收益率输入高度敏感的问题。既可用于资产配置，也可用于多因子模型中的因子权重优化。"
tags: [量化, 资产配置, 策略, 因子, 宏观, 深度研究, black-litterman模型, 均值方差模型, 券商研报, 入门, python, 固收量化]
sources:
  - "[[资产配置风险平价与bl模型]]"
  - "[[主观观点如何融入量化因子模型]]"
  - "[[国君配置资产配置研究进入二次成长期长期配置研究思考与展望]]"
  - "[[板块配置轮盘之初探]]"
  - "[[手把手教你实现black-litterman模型-大类资产配置量化模型研究系列之二]]"
origin: agent-compiled
status: mature
created: 2026-05-21
updated: 2026-06-01
review_by: ""
---
# Black-Litterman模型

## Definition / 定义

Black-Litterman模型（BL模型）由Black和Litterman于1992年提出，是一种在贝叶斯框架下进行资产配置的方法。与传统均值方差模型直接输入预期收益率不同，BL模型从先验权重反推出先验收益率，再结合投资者的观点（views）合成后验收益率分布，最后输入均值方差模型计算最优权重。

## How It Works / 工作原理

### 第一步：计算先验收益率

假设资产收益率服从正态分布，在均值方差模型下从先验权重反推先验收益率：

$$\Pi = \delta \Sigma \omega_{prior}$$

其中 $\Pi$ 为先验收益率，$\delta$ 为风险厌恶系数，$\Sigma$ 为协方差矩阵，$\omega_{prior}$ 为先验权重。

传统BL模型使用各资产的**市值占比**作为先验权重。

### 第二步：形成观点

观点矩阵 $P \cdot Q + \epsilon$，其中 $P$ 为观点系数矩阵，$Q$ 为预测值，$\Omega$ 为观点不确定性矩阵。

### 第三步：贝叶斯合成后验

$$\mu_{BL} = [(\tau\Sigma)^{-1} + P^T\Omega^{-1}P]^{-1}[(\tau\Sigma)^{-1}\Pi + P^T\Omega^{-1}Q]$$

其中 $\tau$ 为先验的不确定性参数。

### 第四步：均值方差最优化

将后验收益率 $\mu_{BL}$ 输入均值方差模型计算各资产权重。

## Key Advantage / 核心优势

- **解决收益率难预测**：不需要直接预测收益率，而是从先验权重反推
- **解决敏感性高**：只有观点涉及的资产权重变化，不涉及的保持不变（局部调整特性）
- **贝叶斯框架**：自然融合市场均衡信息与投资者主观判断

## Improvement in A-Share Context / A股环境下的改进

传统BL模型用**市值权重**作为先验，但在A股市场：
- 总市值受市场表现波动大
- 新股频发不断扩充总市值

[[方正金工]]在[[资产配置风险平价与bl模型]]中提出用[[风险平价]]权重替代市值权重作为先验——既能有效分散风险，又利用了资产历史表现信息。

## Application to Factor Weighting / 因子权重优化中的应用

[[方正金工]]严佳炜在[[主观观点如何融入量化因子模型]]中将BL模型从资产配置领域移植到**多因子选股框架**。核心类比：

- **因子IC值** 类比于 资产收益率
- **因子IC值的协方差矩阵** 类比于 资产风险（协方差矩阵）
- 先验权重由[[风险平价]]确定（基于IC协方差）
- 主观观点来源：因子[[因子动量|动量]]（过去12月IC均值）、因子估值（多头组BP z-score）、个人主观判断

关键实证结果（中证500成分股，6因子，2009-2019）：
- 因子动量观点（加等权±4%约束）：IC 9.68%，IR 2.5，夏普从1.96→2.17
- 动量+估值+个人观点综合：夏普比从1.96（等权）提升至**2.28**
- 特征："部分年度小幅跑输，部分年度大幅跑赢"，沪深300和全A均验证有效

## Related Pages / 关联页面

- [[风险平价]] — 作为BL模型先验权重的改进来源
- [[资产配置风险平价与bl模型]] — RPBL融合策略
- [[主观观点如何融入量化因子模型]] — B-L模型在因子权重优化中的应用
- [[基本面量化]] — B-L模型是基本面量化的核心工具
- [[因子动量]] — 主观观点的来源之一
- [[方正金工]] — 提出RPBL改进和因子权重应用的团队

- [[资产配置演进四阶段]] — 将B-L模型归为理论与现实碰壁期代表
- [[配置三要素]] — 弱者思维与B-L模型贝叶斯框架的关联
- [[板块配置轮盘]] — 宏观四象限下的风格/行业轮动规律工具
- [[宏观四象限]] — 衰退/复苏/过热/滞胀四象限划分
- [[均值方差模型]] — 由马科维茨提出的现代投资组合理论基石，通过均值刻画预期收益、方差刻画风险，寻求在给定风险下收益最大化的最优资产组合。

## 补充发现 / Additional Findings

### 来自 [[手把手教你实现black-litterman模型-大类资产配置量化模型研究系列之二]]

## 缺点 / Cons

尽管BL模型具有诸多优势，但在实际应用中也存在以下局限性：
- **高度依赖主观观点**：模型效果高度依赖主观观点的准确性。
- **观点输入单一**：观点输入方式较为单一，通常需要具体的收益预测。
- **参数设定主观**：参数较多（如风险厌恶系数 $\lambda$、信心水平 $\Omega$），设定缺乏统一标准。
- **分布假设不符现实**：假设收益率服从正态分布，与实际市场的尖峰厚尾特征不符。

## Sources / 来源

- [[手把手教你实现black-litterman模型-大类资产配置量化模型研究系列之二]]
## Sources / 来源

- [[资产配置风险平价与bl模型]] — 方正金工（宋家骥、严佳炜）2019.12
- [[主观观点如何融入量化因子模型]] — 方正金工（严佳炜）2020.04

## Notes / 笔记

<!-- human:start -->
<!-- human:end -->
